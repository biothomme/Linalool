...
def load_arguments():
    ...
    parser.add_argument("-g",
                        ...
                        dest="gene_file",
                        required=False,
                        help="Input a csv_file.")
    parser.add_argument("-e",
                        action="store",
                        dest="edge_file",
                        required=False,
                        help="Input an edge_file.")
    parser.add_argument("-n",
                        action="store",
                        dest="node_file",
                        required=False,
                        help="Input a node_file.")
    parser.add_argument("-t",
                        action="store",
                        dest="tree_file",
                        required=False,
                        help="Input a tree_file.")
    
    args = parser.parse_args()

    return args.csv_file, args.meta_file, args.gene_file, args.edge_file, args.node_file, args.tree_file
...
def assign_shapes_or_col(groups_for_shapes, colors=False):
    ...
    # all_markers = sort_mixed_list([el for el, nm in Line2D.markers.items()
    #                       if nm != 'nothing'])
    all_markers = ['^', 'o', '*', '+']
    if colors:
...
def all_edges_with_coords(std_scent_data,
                          assignments,
                          meta_data,
                          pcomp_a,
                          pcomp_b,
    ...
    LEVELS = [1, .6, .4, .2]
    nod_dat = pd.DataFrame(data=[node_data.index, [v for va in node_data.copy().values for v in va]],
                           index=['V1', 'V2']).transpose()
    center_points, tip_points, tip_edges = get_centered_points(std_scent_data,
                                                               assignments,
                                                               meta_data,
                                                               pcomp_a,
                                                               pcomp_b,
    ...
    # build a alpha dict
    alpha_dict = {cp: 0 for cp in center_points}
    alpha_dict.update({cp: .2 for cp in tip_points})
    ...
    # concat all edges and nodes
    edges_data = pd.concat([nod_dat, tip_edges], axis=0)

    center_points.update(tip_points)
    new_edges = edges_data.copy()
    for i, ed in edges_data.iterrows():
        new_edges.loc[i, :] = [center_points[ed.iat[0]],
                               center_points[ed.iat[1]]]
    new_edges.index = edges_data['V2'].values
    return new_edges, alpha_dict


# plot the tree for all nodes
def plot_tree(edges, alpha_dict, fig, col_dic, cmap):
    import matplotlib.pyplot as plt

    ax = plt.subplot(1, 1, 1)
    for i, row in edges.iterrows():
        xes = [x[0] for x in row]
        yes = [x[1] for x in row]
        # col = col_dic[row.name]

        col = 'black'
        lw = .8
        if '*' in i:
            lw = .2
        ax.plot(xes, yes, c=col, lw=lw, alpha=alpha_dict[row.name])
    return ax
...
def pca_snsplot(std_scent_data,
    ...
    COLOR_MAP = ['Blues', 'Greens', 'Oranges', 'RdPu', 'autumn']#'tab20'

    col_assignment1 = {ass: COLOR_MAP[i] for i, ass in enumerate(assignments.unique())}
    places = meta_data['place_key']
    ...
    col_dic = {asg: co for asg, co in zip(assignments.keys(), colors)}
    # cmap = cm.get_cmap(COLOR_MAP, 256)
    cmap = {co: cm.get_cmap(co, 256) for co in COLOR_MAP}
    col_assignment = {ass: cmap[cmp] for ass, cmp in col_assignment1.items()}

    color_order = []
    cmap_order = []
    for i, colo in enumerate(colors):
        sel_data = meta_data.iloc[i, :]
        ass = sel_data['population']
        sel_cmap = col_assignment1[ass]
        sel_pops = meta_data.loc[meta_data['population'] == ass,
                                 'place_key'].unique()
        sel_colval = [(j+1) / (len(sel_pops)) for j, el in enumerate(sel_pops)
                      if el == sel_data['place_key']][0]
        color_order.append(sel_colval)
        cmap_order.append(sel_cmap)

    cmap_dic = {asg: col_assignment[asg](.5) for asg in assignments.values}
    ...
    if tree:
        edges, alpha_dict = all_edges_with_coords(std_scent_data,
                                                  scent_data.index.values,
                                                  meta_data,
                                                  pcomp_a,
                                                  pcomp_b,
                                                  node_data,
                                                  edges_data)
        ax = plot_tree(edges, alpha_dict, fig, col_dic, cmap=cmap)
    else:
        ax = plt.subplot(1, 1, 1)
    ...
    sif = {nim: nim[1:3] for nim in meta_data.index}
    sif2 = [sf for _, sf in sif.items() if
            sum([1 for el in sif.values() if el == sf]) >3]
    single_ind_free = [[i for i, nm in enumerate(sif.values())
                        if nm == nim] for nim in sif2]
    single_ind_free = [si for sin in single_ind_free for si in sin]
    single_ind_free = [ky for i, ky in enumerate(sif.keys())
                       if i in single_ind_free]

    pca_hybrid_free = pca_hybrid_free.loc[single_ind_free, :].copy()
    ...
    if directions:
        ax = pca_directiplot(std_scent_data,
                             ...
                             ax)
    if phylo:
        # ax = pet.plot_tree(tree=Tree(phylo), axe=ax)
        all_col_dic = {p: cmap[com](co,) for p, co, com in sorted(set(zip(places, color_order, cmap_order)))}
        treee = Phylo.read(StringIO(phylo), "newick").as_phyloxml()

        label_colors = phylo.replace('(', '').replace(')', '').replace(';\n', '')
        label_colors = [all_col_dic[spi] for spi in label_colors.split(',')]

        Phylo.draw(treee, axes=ax, do_show=False, label_colors=all_col_dic)
        

    # plot the points
    if directions != 'solely':
        for i, (_, data_row) in enumerate(pca_plotting_data.iterrows()):
            # cur_cmap = col_assignment1[data_row['colors']]
            ax.scatter(x=data_row[pc_a],
                       y=data_row[pc_b],
                       c=color_order[i],
                       cmap=cmap_order[i],
    ...
    if not isinstance(gene_file, type(None)):
        ...
        pie_coords = trafo_ax_fig(ax, [12.5, -3])
    ...
    if directions != 'solely':
        ...
        for i, pops in enumerate(assignments.unique()):
            handles = list()
            plc_data = meta_data.loc[meta_data['population'] == pops, :]
            plcs = list(plc_data['place_key'].unique())
            for ass, col, ccmp in sorted(set(zip(places, color_order, cmap_order))):
                if ass in plcs:
                    # cur_cmap = col_assignment[ass]
                    new_handle = ax.plot([], [], lw=2, c=cmap[ccmp](col,), label=ass)
                    handles.append(new_handle[0])

            legend1 = ax.legend(handles=handles,
                                bbox_to_anchor=(old_x, old_y),
                                loc='upper left',
                                borderaxespad=0.,
                                title=pops)
            bbox = print_to_string(legend1.get_bbox_to_anchor())

            new_x, new_y = legposnew(bbox)
            old_y = old_y - new_y*.2
...
def legposnew(bbox_string):
    import re
    nums = r'[0-9]*\.[0-9]*'
    bbox_lines = bbox_string.split('\n')
    the_line = bbox_lines[4]
    equals = the_line.split('=')
    x0 = float(re.search(nums, equals[-4])[0])
    y0 = float(re.search(nums, equals[-3])[0])
    x1 = float(re.search(nums, equals[-2])[0])
    y1 = float(re.search(nums, equals[-1])[0])
    return x1-x0, y1-y0
...
def obtain_groups(scent_data, meta_data):
    ...
    meta_bool = [i for i, (j, row) in enumerate(meta_data.iterrows())
                 if row['florscent_key'] in specimen]
    meta_selection = meta_data.iloc[meta_bool, :]

    population = [plo[1:3] for plo in meta_selection.index]

    meta_selection.loc[:, 'population'] = population
...
def sort_by_same_order(scent_data, meta_data):
    ...
    meta_data = meta_data.loc[meta_data['florscent_key'].isin(scent_data.index), :].copy()
...
# fingers of the tree
def tree_fingers(std_scent_data,
                 assignmente,
                 coordinates):
    import pandas as pd

    assignments = set(coordinates['class'].values)

    points = {}
    for specimen in assignmente:
        coords = coordinates.loc[coordinates['name'] == specimen, ('x', 'y')]
        points[specimen] = coords.values[0]

    finger_frame = pd.DataFrame([], columns=['V1', 'V2'])
    for i, ass in enumerate(assignments):
        sel_sps = coordinates.loc[coordinates['class'] == ass, 'name'].values
        for j, sp in enumerate(sel_sps):
            finger_frame.loc[f'{i}-{j}', :] = [ass, sp]
    return points, finger_frame


def get_centered_points(std_scent_data,
                        assignmente,
                        meta_data,
                        pcomp_a,
                        pcomp_b,
                        ...
                        edges_data=None):
    '''
    get centered points for populations by taking the center between specimen
    Parameters
    ----------
    std_scent_data , meta_data , pcomp_a : num of principal comp;
    pcomp_b ,  node_data : optional - for plotting all nodes

    Returns
    -------
    center_points : dict with coordinates for each node

    '''
    import pandas as pd
    from sklearn.decomposition import PCA

    pca = PCA().fit(std_scent_data)

    assignments = set(meta_data['place_key'].values)

    pca_values = pca.transform(std_scent_data)
    coordinates = pd.DataFrame(zip(assignmente,
                                   meta_data['place_key'].values,
                                   pca_values[:, pcomp_a-1],
                                   pca_values[:, pcomp_b-1]),
                               columns=['name', 'class', 'x', 'y'])

    center_points = {}
    # for tiplabels
    for ass in assignments:
        sel_coords = coordinates.loc[coordinates['class'] == ass, ('x', 'y')]
        center_points[ass] = centerrr(sel_coords).values

    # for nodes
    node_data.sort_index(ascending=False, inplace=True)

    node_data2 = node_data.copy()
    for co in range(3):
        for ass in node_data2.index.unique():
            # ass = str(row.name)
            populations = sorted([rel for _, rw in
                           node_data.iterrows() if rw.name == ass for rel in
                           rw['V2'].split('-')])
            judge = [False for el in populations if 'Node' in el]
            if len(judge) == 0:
                node_data.loc[node_data['V2'] == ass] = '-'.join(populations)
    ...
    for ass in node_data2.index.unique():
        ...
        sel_coords = pd.DataFrame([], columns=sel_coords.columns)
        for j, pop in enumerate(populations):
            sel_coords.loc[str(j), :] = center_points[pop]
        center_points[ass] = centerrr(sel_coords).values
    ...
    for k in range(100):
        for ass in node_data2.index.unique():
            ...
            center_points['Node2'] = np.array([0, 0])
    tip_points, tip_edges = tree_fingers(std_scent_data, assignmente, coordinates)

    return center_points, tip_points, tip_edges



# return the neigbours of a node.
def get_neighbours(node, edges_data):
    neighs = []
    for i, row in edges_data.iterrows():
        neigh = [el for el in row if el != node]
        if len(neigh) == 1:
            neighs += neigh
    return neighs


def centerrr(x_and_y_coords):
    '''
    Parameters
    ----------
    x_and_y_coords : a pd df with x and y coordinates in 2 columns

    Returns
    -------
    TYPE
        mean of x and y in df
    '''
    return x_and_y_coords.mean(axis=0)
...
def plt_bray_curtis(dist_matrix, meta_data):
    ...
    dendrog_frame = sch.linkage(dist_matrix, method='complete')
    ...
    coldic = {cdk: cdv / max(list(coldic.values())) for cdk, cdv in
              coldic.items()}
    ...
    for y in xlbls:
        y.set_color(cmap(coldic[y.get_text()]))
...
def pca_run(std_scent_data,
    ...
    std_scent_data = imp.transform(std_scent_data).transpose()
...
csv_file, meta_file, gene_file, node_data, edges_data, tree_file = load_arguments()
...
if tree_file:
    with open(tree_file, 'r') as tf:
        tree_str = tf.readlines()        
        tree_str = str(tree_str[0])

if node_data:
    node_data = load_matrix(node_data, infer_header=True, sep=',')

if edges_data:
    edges_data = load_matrix(edges_data,
                             infer_header=True,
                             sep=',',
                             first_col=True)
